...
## Current State
Full-stack application with PostgreSQL backend, Express API, and React frontend. Version 1.15.0 — Season progression (bonus _add → permanent XP), bot auto-setup (lineup/rotation/tactics), tactic entries in play log, opponent tactics preview, red countdown, customizable team color, compact TeamPage. Seeker MWA support (v1.14.2). Home & Team UI/UX (v1.14.1). Dynamic Tactic Coefficients (v1.14.0). Per-pitcher pitching staff (v1.13.0).
