...
## Current State
Full-stack application with PostgreSQL backend, Express API, and React frontend. Version 1.17.1 — Lineup order fix (useEffect race condition), unsaved changes indicator, simulation logging, batting order display in match detail (v1.17.1). Stolen base mechanic (2-phase: attempt + success), DB coefficients migration (attack_style, defense_counter layers, tac_st column), dynamic coefficient badges on Attack/Defense pages (v1.17.0). Pitcher tactics in play log, data loading fix (v1.16.0). Login crash prevention (v1.15.1). Season progression (v1.15.0). Seeker MWA support (v1.14.2). Dynamic Tactic Coefficients (v1.14.0). Per-pitcher pitching staff (v1.13.0).
