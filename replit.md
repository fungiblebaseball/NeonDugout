...
## Current State
Full-stack application with PostgreSQL backend, Express API, and React frontend. Version 1.18.0 — Image unity overhaul: fix inter-league promotion bug, welcome animations (confetti/banners), neon style alignment across all pages, collapsible Defense/Admin sections, CoeffBadges in PitchersPage, projected playoff previews in Schedule, admin messaging system (v1.18.0). Lineup order fix, unsaved changes indicator (v1.17.1). Stolen base mechanic, DB coefficients migration (v1.17.0). Pitcher tactics in play log, data loading fix (v1.16.0). Login crash prevention (v1.15.1). Season progression (v1.15.0). Seeker MWA support (v1.14.2). Dynamic Tactic Coefficients (v1.14.0). Per-pitcher pitching staff (v1.13.0).
